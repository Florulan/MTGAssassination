...
from gestion_des_decks import charger_decks
...
def saisir_joueurs_et_decks():
    tous_les_joueurs = charger_joueurs()
    tous_les_decks = charger_decks()
    joueurs_choisis = []
    decks_par_joueur = {}
    ...
    while True:
        ...
        if choix.isdigit() and 1 <= int(choix) <= len(restants):
            joueur = restants[int(choix) - 1]
            joueurs_choisis.append(joueur)

            print(f"\nQuel deck utilise {joueur} ?")
            deck_choisi = afficher_choix(tous_les_decks)
            decks_par_joueur[joueur] = deck_choisi
        else:
            print("❌ Choix invalide.")

    return joueurs_choisis, decks_par_joueur
...
def sauvegarder_partie(kills_order, ordre_morts, scores, decks_par_joueur):
    partie = {
        "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "kills_order": kills_order,
        "ordre_morts": ordre_morts,
        "scores": scores,
        "decks": decks_par_joueur
    }
...
# --- Lancement du jeu ---
joueurs, decks_par_joueur = saisir_joueurs_et_decks()
...
for joueur, score in scores.items():
    print(f"{joueur} : {score} points (deck : {decks_par_joueur.get(joueur)})")

sauvegarder_partie(kills_order, ordre_morts, scores, decks_par_joueur)
